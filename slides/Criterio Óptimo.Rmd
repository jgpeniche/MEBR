...
Ahora, gracias al axioma V sabemos que podemos una región lo suficientemente **creible** *.,. nos de lo mismo* que una opción **segura**, esto es:
...
Si ocurre $E_1$ ó $E_1^c$ $d$ y $d^{(1)}$ son igualmente preferibles
...
$d_1^{(k)} \prec d_1^{(k)}$ $\Longleftrightarrow$ $A_1$ es **más** *creible* que $A_2$
...
$$= \sum_{i=1}^{k_i} P(E_{j,i}) P( R_{j,i}|E_{j,i})$$ Por independencia
