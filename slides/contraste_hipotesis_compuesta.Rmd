...
$$\Lambda = \frac{l_j \int_{A}  P(\theta | \underline{X}_{(n)}) \partial \theta}{l_i \int_{A^c} P(\theta | \underline{X}_{(n)}) \partial \theta} = \frac{l_j P(\theta \epsilon A)\int_{A}  P(\theta | \underline{X}_{(n)})P(\theta | \theta \epsilon A) \partial \theta}{l_i P(\theta \epsilon A^c) \int_{A^c} P(\theta | \underline{X}_{(n)})P(\theta | \theta \epsilon A^c) \partial \theta}< 1$$
--
...
# Contraste de Hipótesis (Simple contra Compuesta)
